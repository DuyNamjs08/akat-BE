...
const fetchPostsCount = async (
  ...
  let url = `https://graph.facebook.com/v22.0/${page_id}/posts?fields=id,message,created_time,shares,likes.summary(true).limit(0),comments.summary(true).limit(0),full_picture,status_type&since=${since}&until=${today}&limit=10&access_token=${token}`;
  try {
    while (url) {
...
const updateInsight = async (data: any) => {
  try {
    const nameAndImage = `https://graph.facebook.com/v22.0/${data?.id}?fields=name,picture&access_token=${data?.longLivedPageToken}`;
    const followersUrl = `https://graph.facebook.com/v22.0/${data?.id}/insights?metric=page_daily_follows_unique&period=days_28&access_token=${data?.longLivedPageToken}`;
    const postRemain = `https://graph.facebook.com/v22.0/${data?.id}/insights?metric=page_impressions_unique,page_post_engagements&period=days_28&access_token=${data?.longLivedPageToken}`;
    const categoryAndStatusPage = `https://graph.facebook.com/v22.0/${data?.id}?fields=category%2Cis_published&access_token=${data?.longLivedPageToken}`;
    ...
    console.error('Error Sync Facebook insight', {
    ...
    const targetJob = repeatableJobs.find(
      (job) => job.id === 'facebook-sync-user_123',
    );
    if (targetJob) {
...
facebookInsightQueue.process(async (job) => {
  const { data } = job;
